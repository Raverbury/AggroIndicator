...
public class AggroIndicatorClient implements ClientModInitializer {
    ...
    private void registerClientEventHandlers() {
        WorldRenderEvents.AFTER_ENTITIES.register((worldRenderContext) -> {
            ...
            List<MobEntity> mobs = world.getEntitiesByClass(MobEntity.class, player.getBoundingBox().expand(32),
                    (mob) -> true);

            for (MobEntity mob : mobs) {
                if (mob.hasStatusEffect(StatusEffects.INVISIBILITY) || mob.isInvisible()) {
                    continue;
                }
            ...
            AlertRenderer.renderAlertIcon(worldRenderContext.tickDelta(), worldRenderContext.matrixStack(),
                    worldRenderContext.camera());
        });
    }

    private void registerClientPacketHandlers() {
        ClientPlayNetworking.registerGlobalReceiver(S2CMobChangeTargetPacket.PACKET_TYPE,
